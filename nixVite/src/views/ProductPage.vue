...
const selectedImage = ref('')
    ...
    // Загрузка данных
    onMounted(async () => {
        // 1. Получаем сырой ID из параметров маршрута
        const rawId = route.params.id

        // 2. Приводим к строке: если это массив, берем первый элемент, иначе саму строку
        // Если rawId undefined, то id тоже будет undefined
        const id = Array.isArray(rawId) ? rawId[0] : rawId

        // 3. Проверяем, что ID существует
        if (!id) {
            router.push('/catalog')
            return
        }
        ...
        try {
            ...
            // 4. Вызов API с гарантированно строковым ID
